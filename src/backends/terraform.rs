...
impl TerraformBackend {
    ...
    pub fn init_command(&self, session: &BackendSession) -> StepCommand {
        ...
        // In isolated mode, the backend override file forces local backend,
        // so init runs normally without -backend=false.
        command
    }
    ...
    fn materialize_request(
        ...
        // This overrides any remote backend configuration in the original HCL.
    ...
    fn write_backend_override(&self, working_directory: &Path) -> io::Result<()> {
        // Write the Terraform override file to force local backend.
        // This overrides any backend block in the original HCL.
        // Backend partial config files (backend.auto.hcl, *.auto.tfbackend) are
        // excluded during workspace copy, so they won't conflict.
        let override_path = working_directory.join("dress_backend_override.tf");
        let tf_content = r#"# Generated by dress-rehearsal for isolated rehearsal mode.
# This file overrides any backend configuration to use local state only.
...
"#;
        fs::write(override_path, tf_content)
    }
...
fn copy_deployment_tree(source: &Path, destination: &Path) -> io::Result<()> {
    ...
    for entry in fs::read_dir(source)? {
        ...
        // Skip directories that should not be copied to the isolated workspace
        if file_name == ".terraform" || file_name == ".dress-runs" || file_name == ".git" {
            continue;
        }
        // Skip backend partial configuration files that would conflict with the
        // local backend override. These typically contain remote backend settings
        // (bucket, prefix, etc.) that are invalid for the local backend.
        if file_name == "backend.auto.hcl" || file_name.ends_with(".auto.tfbackend") {
            continue;
        }
...
mod tests {
    ...
    fn initializes_backend_session_and_materializes_workspace() -> io::Result<()> {
        ...
        // Backend partial config file that should be excluded from the isolated workspace
        fs::write(
            scenario_root.join("backend.auto.hcl"),
        ...
        // Backend partial config files should be excluded to avoid conflicts with local backend
        assert!(
            !session.deployment_root().join("backend.auto.hcl").exists(),
            "backend.auto.hcl should be excluded from isolated workspace"
        );
        assert_eq!(
    ...
    #[test]
    fn isolated_commands_use_local_state_path() {
        ...
        // Init runs without special flags; backend override file handles isolation
        assert_eq!(init_command.args(), &["init".to_string()]);
        // Apply/output/destroy use explicit state path for the local backend
