...
"#;
        ...
        // Also write an empty backend.auto.hcl to prevent Terraform from picking
        // up backend partial configurations from parent directories.
        let auto_hcl_path = working_directory.join("backend.auto.hcl");
        let hcl_content = r#"# Generated by dress-rehearsal for isolated rehearsal mode.
# This empty file prevents Terraform from loading backend.auto.hcl from parent directories.
# DO NOT COMMIT THIS FILE.
"#;
        fs::write(auto_hcl_path, hcl_content)?;
...
mod tests {
    ...
    fn initializes_backend_session_and_materializes_workspace() -> io::Result<()> {
        ...
        // Backend override files should be generated in the working directory
        ...
        // Empty backend.auto.hcl should prevent parent directory configs
        let auto_hcl_path = session.working_directory().join("backend.auto.hcl");
        assert!(
            auto_hcl_path.exists(),
            "backend.auto.hcl should exist to block parent configs"
        );
        Ok(())
    }
